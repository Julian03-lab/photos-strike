...
const ExtraDataScreen = () => {
  ...
  const submitEnabled = gender && country && birthday;
  ...
  const onSubmit = async () => {
    if (!session || !gender || !birthday || !country) return;
    ...
    const data: { [key: string]: string | Date | null } = {
      ...
      gender,
      photoURL: photoURL ? photoURL : session.photoURL,
    };
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {/* Save button */}
        <View style={!submitEnabled && { opacity: 0.6 }}>
          <PrimaryButton
            onPress={onSubmit}
...
export default ExtraDataScreen;
